<?php
    class PrototypeNModel extends Mysql
    {
        public $strName;
        ...
        {
            $sql = "SELECT * FROM prototype_nutricionista";
        ...
        public function insertarPrototipo(string $name, string $description)
        {
            ...
            $sql = "INSERT INTO prototype_nutricionista (name, description) VALUES (?, ?)";
        ...
        public function getPrototype(int $id){
            $this->intId = $id;
            $sql = "SELECT * FROM prototype_nutricionista WHERE id=$this->intId";
        ...
        public function updatePrototype(int $id, string $name, string $description)
        {
            ...
            $this->strName = $name;
            $this->strDescription = $description;
            $sql = "SELECT * FROM prototype_nutricionista WHERE name='$this->strName' AND id!=$this->intId";
            $requestSelect = $this->select($sql);
            if(empty($requestSelect)){
                $sqlUpdate = "UPDATE prototype_nutricionista set name=?, description=? WHERE id=$this->intId";
        ...
        {
            $this->intId = $id;
            $preSql = "SELECT * FROM prototype_nutricionista WHERE id= $this->intId";
            $request = $this->select($preSql);
            if(empty($request)){
                $response ='error';
            }else{
                $sql = "DELETE FROM prototype_nutricionista WHERE id=$this->intId";
